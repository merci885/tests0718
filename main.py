...
if selected_mbti and selected_mbti != "선택해주세요":
    ...
    mbti_emojis = {
        ...
        "ISTJ": "⚖️🏛️", "ISFJ": "🤗🌺", "ESTJ": "🏆📋", "ESFJ": "🎉👥",
        "ISTP": "🔧⚙️", "ISFP": "🎨🌿", "ESTP": "⚡🏃", "ESFP": "🎪🌟"
    }
    
    st.markdown(f'<div class="mbti-badge">{mbti_emojis[selected_mbti]} {selected_mbti}</div>', unsafe_allow_html=True)
    ...
    for i, book in enumerate(books):
        st.markdown(f'''
        <div class="book-card">
            <div class="book-title">{book_emojis[i]} {book["title"]}</div>
    ...
    mbti_descriptions = {
        ...
        "ENFJ": "🌟❤️ 타인을 돕고 사회적 가치를 중시하는 ENFJ는 인간애와 긍정적 변화를 다룬 작품에 감동받습니다.",
        "ENFP": "🌈🎈 열정적이고 낙관적인 ENFP는 희망과 가능성, 모험이 가득한 작품에서 영감을 얻습니다.",
        "ISTJ": "⚖️🏛️ 전통과 안정성을 중시하는 ISTJ는 확고한 가치와 체계적인 접근이 돋보이는 작품을 선호합니다.",
        ...
        "ISTP": "🔧⚙️ 실용적이고 독립적인 ISTP는 기술과 논리, 개인적 도전을 다룬 간결한 작품을 선호합니다.",
...
# 성공 메시지와 추가 풍선 효과
    st.success("🎉 완벽한 책 추천이 완료되었습니다! 즐거운 독서 되세요! 📚✨")
    
    # 추천 완료 후 추가 상호작용
    if st.button("🎈 다른 MBTI도 궁금해요!"):
        st.balloons()
        st.info("🔄 위에서 다른 MBTI를 선택해보세요! 새로운 책들이 기다리고 있어요! 🌟")
...
with st.sidebar:
    ...
    **✨ 독서 팁:**
    ...
    # 사이드바에도 풍선 효과 버튼 추가
    if st.button("🎉 축하 풍선!", key="sidebar_balloon"):
        st.balloons()
        st.success("🎈 축하합니다! 좋은 선택이에요! 🎈")
